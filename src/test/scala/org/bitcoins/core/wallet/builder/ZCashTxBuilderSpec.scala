...
class ZCashTxBuilderSpec extends Properties("ZCashTxBuilderSpec") {
  private val logger = BitcoinSLogger.logger
  val timeout = 5.seconds
  property("sign a mix of spks in a tx and then have it verified") = {
    Prop.forAllNoShrink(CreditingTxGen.zCashOutputs) {
      case creditingTxsInfo =>
        ...
        Prop.forAll(TransactionGenerators.smallOutputs(totalAmount), ScriptGenerators.scriptPubKey, ChainParamsGenerator.zcashNetworkParams) {
          case (destinations: Seq[TransactionOutput], changeSPK, network) =>
            ...
            val signedTx = builder.flatMap(_.sign)
            val result = signedTx.map { tx =>
  ...
  private def buildCreditingTxInfo(info: Seq[CreditingTxGen.CreditingTxInfoZcash]): ZcashTxBuilder.UTXOMap = {
    @tailrec
    def loop(
      rem: Seq[CreditingTxGen.CreditingTxInfoZcash],
      accum: ZcashTxBuilder.UTXOMap): ZcashTxBuilder.UTXOMap = rem match {
      case Nil => accum
      case (tx, idx, signers, redeemScriptOpt, scriptWitOpt, hashType) :: t =>
        ...
        val outPointsSpendingInfo = ZcashUTXOSpendingInfo(o, output, signers, redeemScriptOpt, scriptWitOpt, hashType)
