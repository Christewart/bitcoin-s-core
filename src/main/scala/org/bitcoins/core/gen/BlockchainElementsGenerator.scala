...
import org.bitcoins.core.protocol.blockchain.{ BitcoinBlockHeader, Block, BlockHeader, ZcashBlockHeader }
...
sealed abstract class BlockchainElementsGenerator {
  ...
  private def buildBlockHeader(prevBlockHash: DoubleSha256Digest, nBits: UInt32): BlockHeader = {
    ...
    BitcoinBlockHeader(UInt32.one, prevBlockHash, EmptyTransaction.txId, UInt32.one, nBits, nonce)
  }
}
