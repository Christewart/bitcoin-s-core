...
case class P2PClientActor(
  ...
  private def handleEvent(
    ...
    event match {
      case Tcp.Bound(localAddress) =>
        logger.info(s"Actor is now bound to the local address: ${localAddress}")
      ...
      case Tcp.Connected(remote, local) =>
        logger.info(s"Tcp connection to: ${remote}")
  ...
  /**
    * Returns the current state of our peer given the [[P2PClient.MetaMsg]]
    * @param metaMsg
    * @return
    */
...
case class P2PClient(actor: ActorRef, peer: Peer) {
  ...
  def isInitialized()(
    ...
    isInitF.recoverWith { case _: Throwable => Future.successful(false) }
  }
  ...
  def isDisconnected()(
    ...
    isDisconnect.recoverWith { case _: Throwable => Future.successful(true) }
  }
}
...
object P2PClient {
  ...
  case object IsConnected extends MetaMsg

  case object IsInitialized extends MetaMsg

  case object IsDisconnected extends MetaMsg
