...
object BitcoinSServer {
  private[server] var startedFP = Promise[Future[ServerBindings]]()
  ...
  /** Allows the above server to be bundled with other projects.
    ...
    * been initialized.
    */
  val startedF: Future[ServerBindings] = startedFP.future.flatten

  def reset(): Unit = {
    startedFP = Promise[Future[ServerBindings]]()
  }
}
